require "jwt"
require "uri"
require "./index_constants"
require "./index_types"
require "./index_storage"
require "./errors"
...
class LogtoClientClass
  ...
  def initialize(config? : LogtoClient::Config, navigate? : Proc, storage? : LogtoClient::AbstractStorage, cache? : LogtoClient::AbstractStorage = CrystalLruCache.new(app_id: config.app_id) )
    ...
    @access_token_map = @storage.get(STORAGE_KEY[:access_token_map]) || Hash{String, String}
  end
  ...
  def sign_in(redirect_uri? : String, first_screen? : String = nil, login_hint? : String = nil, direct_sign_in? : Hash = nil, post_redirect_uri? : String = nil, extra_params? : Hash = nil)
  ...
  def sign_out(post_logout_redirect_uri? : String = nil)
    if refresh_token
  ...
  def handle_sign_in_callback(url? : String)
  ...
  def access_token(resource? : String = nil, organization_id? : String = nil)
  ...
  def access_token_claims(resource? : String = nil, organization_id? : String = nil)
  ...
  def fetch_user_info
    return @core.fetch_user_info(access_token: access_token)
  end
  ...
  def refresh_token
    return @storage.get(STORAGE_KEY[:refresh_token])
  end
  ...
  def clear_all_tokens
    @access_token_map = Hash{ String, String }
    @storage.remove(STORAGE_KEY[:access_token_map])
    @storage.remove(STORAGE_KEY[:id_token])
    @storage.remove(STORAGE_KEY[:refresh_token])
  end

  protected def handle_token_response(response : LogtoCore::TokenResponse, resource : String? = nil, organization_id : String? = nil)
      raise ArgumentError.new("Response must be a TokenResponse") unless response.is_a?(LogtoCore::TokenResponse)
  ...
  def save_refresh_token(token : String)
      raise ArgumentError, "Token must be a String" unless token.is_a?(String)
    @storage.set(STORAGE_KEY[:refresh_token], token)
  end

  def save_id_token(token : String)
      raise ArgumentError, "Token must be a String" unless token.is_a?(String)
    @storage.set(STORAGE_KEY[:id_token], token)
  end

  def save_access_token(key? : String, token? : LogtoCore::AccessToken)
  ...
  def save_sign_in_session(data : SignInSession)
  ...
  def fetch_jwks(options : Hash = {String, String})
    if options[:kid_not_found] && ((@cache.get("jwks_last_update") || 0) < Time.now.to_i - 300)
    @cache.remove("jwks")
    end

    jwks_hash = @cache.get("jwks") || begin
      response = JSON.parse(HTTP::Client.get(URI.parse(@core.oidc_config[:jwks_uri])))
