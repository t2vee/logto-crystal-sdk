...
class LogtoError < Exception
  ...
  #
  # @attr [HTTP::ClientResponse, nil] response The response object that caused this error.
  class ResponseError < LogtoError
    attr_reader :response

    def initialize(message, response? : HTTP::Client::Response = nil)
      raise ArgumentError, "response must be a HTTP::ClientResponse or nil" unless response.nil? || response.is_a?(HTTP::Client::Response)
      super(message)
      @response = response
    end
  end
