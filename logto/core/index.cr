...
class LogtoCoreClass
  ...
  def revoke_token(client_id, client_secret, token)
    response = HTTP::Client.post_form(
    ...
    raise LogtoError::RevocationError.new(response.message, response: response) unless
      response.is_a?(HTTP::ClientSuccess)
  end
  ...
  def fetch_user_info(access_token? : String)
    uri = URI.parse(oidc_config.userinfo_endpoint)
    request = HTTP::Client::Get.new(uri)
    request["Authorization"] = "Bearer #{access_token}"

    response = HTTP::Client.start(uri.host, uri.port, use_ssl: uri.scheme == "https") do |http|
  ...
  def generate_sign_in_uri(client_id? : String, redirect_uri? : String, code_challenge? : String, state? : String, scopes? : String = nil, resources? : String = nil, prompt? : String = nil, first_screen? : String = nil, interaction_mode? : String = nil, login_hint? : String = nil, direct_sign_in? : String = nil, extra_params? : String = nil, include_reserved_scopes? : Bool = true)
    parameters = {
      QUERY_KEY[:client_id] => client_id,
      QUERY_KEY[:redirect_uri] => redirect_uri,
  ...
  def generate_sign_out_uri(client_id? : String, post_logout_redirect_uri? : String = nil)
    parameters = {
      QUERY_KEY[:client_id] => client_id
    }
